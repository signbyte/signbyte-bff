...
| `GET /api/portal/v1/envelopes/{id}` | session | Composed detail → header (with `origin` `{name, returnUrl?, ref?}` when a document system prepared the envelope — what the app shows as *Requested by …* — absent for an envelope started in the portal) + `slots[]` (each enriched with its live signing state; a signer's own `returnUrl` is forwarded on the viewer's own slot, and on every slot to the owner, never on another party's) + `documents[]` |
