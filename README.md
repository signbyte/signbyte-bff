...
| `POST /api/portal/v1/envelopes` | session + CSRF | Create a draft envelope `{title?, orderPolicy?, profile?, documents?[], slots?[]}` — a slot's `identityRef` may carry a `country` beside it and is relayed in the platform's canonical spelling; it must name a **natural person** — an organisation's identifier is refused `422`, since nobody logs in as an organisation and the slot could never be matched; 2+ `documents` are first bundled into ONE unsigned ASiC-E the envelope references → `{id, status, version, slotIds}` |
...
| `GET /api/portal/v1/envelopes/{id}` | session | Composed detail → header (with `origin` `{name, ref?}` when a document system prepared the envelope — never the requester's default address: a return travels on the signer's own slot — what the app shows as *Requested by …* — absent for an envelope started in the portal) + `slots[]` (each enriched with its live signing state; a signer's own `returnUrl` is forwarded on the viewer's own slot, and on every slot to the owner, never on another party's) + `documents[]` |
| `POST /api/portal/v1/envelopes/{id}/documents` | session + CSRF | Attach a document `{documentId}` (draft-only) |
| `POST /api/portal/v1/envelopes/{id}/slots` | session + CSRF | Add a signer slot `{orderIndex, role?, flow?, requiredLoa?, identityRef?, country?}` → `{id}`. `country` (two letters) supplies the identity code's country when the code itself names none; a code that names one is believed. A code with neither is refused `422`, as is one naming an organisation rather than a person |
...
- **No open-redirect surface** — the login return, logout return, and signing-return URLs are all built from this service's own configuration, never from client-supplied input. The one return address relayed *to* the browser — an envelope origin's, or a signer's own — is data the app renders as a *Return to …* action, never a redirect this service performs: it was admitted by the service that created the envelope (https only, checked against the requesting system's registration), and it is forwarded only to the signer it belongs to, or to the owner.
