...
// readyz is readiness: the session store is reachable. It returns a plain {status}
// body — never an error envelope — so an orchestrator probe gets a uniform
// readiness signal, and skips the access line/span.
func (r *router) readyz(ctx *azugo.Context) {
	ctx.SkipRequestLog()
	if err := r.Sessions().Ping(ctx); err != nil {
		notReady(ctx, "session store")
